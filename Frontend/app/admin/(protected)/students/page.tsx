// File: Frontend/app/admin/(protected)/students/page.tsx
"use client";
...
// --- Add Student Modal ---
function AddStudentModal({ isOpen, onClose, onSave, courses }: { isOpen: boolean; onClose: () => void; onSave: (data: any) => Promise<void>; courses: string[]; }) {
    const [formData, setFormData] = useState({ firstName: '', lastName: '', gender: '', age: '', parentName: '', relationship_with_student: '', email: '', phone: '', whatsapp: '', country: '', course: '', });
...
function ManageStudentModal({ student, teachers, isOpen, onClose, onSave }: { student: Student; teachers: Teacher[]; isOpen: boolean; onClose: () => void; onSave: (data: any) => Promise<void>; }) {
    const [teacherId, setTeacherId] = useState<string>(student.teacher_id?.toString() || '');
    ...
    const handleSubmit = async (e: FormEvent) => {
        e.preventDefault();
        if (!teacherId || !shift) { setError('Please select both a teacher and a shift.'); return; }
        setIsLoading(true);
        setError(null);
        try {
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl max-w-lg w-full">
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="flex justify-end p-4 bg-gray-50 border-t"><button type="button" onClick={onClose} className="px-4 py-2 mr-2 text-sm bg-white border rounded-md hover:bg-gray-100">Cancel</button><button type="submit" disabled={isLoading} className="px-4 py-2 text-sm text-white bg-primary rounded-md hover:bg-primary/90 disabled:opacity-50">{isLoading ? 'Saving...' : 'Approve & Assign'}</button></div>
                </form>
            </div>
        </div>
    );
...
export default function StudentsPageWrapper() {
  return (<Suspense fallback={<div className="p-10">Loading page...</div>}><StudentsPage /></Suspense>);
}
...
function StudentsPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            const [studentsRes, teachersRes] = await Promise.all([
                fetch('http://localhost:8000/admin/students/', { credentials: 'include' }),
                fetch('http://localhost:8000/admin/teachers/', { credentials: 'include' })
            ]);
    ...
    const handleUpdateStudent = async (assignmentData: { teacher_id: number; shift: string; }) => {
        if (!selectedStudent) return;
        const response = await fetch(`http://localhost:8000/admin/students/${selectedStudent.id}/assign`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
            body: JSON.stringify(assignmentData),
        });
        if (!response.ok) { const errorData = await response.json(); throw new Error(errorData.detail || 'Failed to assign student.'); }
    ...
    const filteredStudents = useMemo(() => {
        ...
        if (view === 'unassigned') { currentStudents = currentStudents.filter(s => s.status === 'Pending'); } 
        else {
            currentStudents = currentStudents.filter(s => s.status === 'Approved' || s.status === 'Finished');
        ...
        <div className="p-6 sm:p-10">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-500">Manage students based on their status.</p>
                </div>
                <div className="flex items-center space-x-4">
                    <button onClick={() => setIsAddModalOpen(true)} className="flex items-center px-4 py-2 text-sm text-white bg-primary rounded-lg shadow-sm hover:bg-primary/90"><UserPlus className="w-5 h-5 mr-2" />Add Student</button>
                </div>
            </div>

            <div className="mt-4 flex flex-wrap items-center gap-4">
                <div><label htmlFor="courseFilter" className="block text-sm font-medium">Filter by Course</label><select id="courseFilter" value={courseFilter} onChange={e => setCourseFilter(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 border rounded-md"><option value="all">All Courses</option>{courses.map(course => <option key={course} value={course}>{course}</option>)}</select></div>
                <div><label htmlFor="genderFilter" className="block text-sm font-medium">Filter by Gender</label><select id="genderFilter" value={genderFilter} onChange={e => setGenderFilter(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 border rounded-md"><option value="all">All Genders</option><option value="Male">Male</option><option value="Female">Female</option></select></div>
                {view === 'all' && (<div><label htmlFor="statusFilter" className="block text-sm font-medium">Filter by Status</label><select id="statusFilter" value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 border rounded-md"><option value="all">All</option><option value="Approved">Approved</option><option value="Finished">Finished</option></select></div>)}
                {view === 'unassigned' && (<div><label htmlFor="countryFilter" className="block text-sm font-medium">Filter by Country</label><select id="countryFilter" value={countryFilter} onChange={e => setCountryFilter(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 border rounded-md"><option value="all">All Countries</option>{countries.map(country => <option key={country} value={country}>{country}</option>)}</select></div>)}
            </div>

            <div className="mt-8 bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-x-auto">
                <table className="w-full text-sm text-left">
                    <thead className="text-xs uppercase bg-gray-50"><tr><th className="px-6 py-3">Student Name</th><th className="px-6 py-3">Age</th><th className="px-6 py-3">Gender</th><th className="px-6 py-3">Parent Name</th><th className="px-6 py-3">Phone</th><th className="px-6 py-3">Country</th><th className="px-6 py-3">Course</th><th className="px-6 py-3">Shift</th><th className="px-6 py-3">Assigned Teacher</th><th className="px-6 py-3">Status</th><th className="px-6 py-3">Action</th></tr></thead>
                    <tbody>
                ...
                {filteredStudents.length === 0 && <div className="text-center p-8">No students match the current filters.</div>}
            </div>

            <AddStudentModal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} onSave={handleSaveNewStudent} courses={courses} />
            {selectedStudent && <ManageStudentModal isOpen={!!selectedStudent} onClose={() => setSelectedStudent(null)} student={selectedStudent} teachers={teachers} onSave={handleUpdateStudent} />}
        </div>
    );
}